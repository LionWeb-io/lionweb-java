...
public class LionWebArchive {
  ...
  public static void load(
      ...
      Consumer<SerializationChunk> chunkConsumer)
      throws IOException {
    if (!file.exists()) {
  ...
  public static void load(File file, JsonSerialization jsonSerialization) throws IOException {
    load(
        file,
        jsonSerialization.getLionWebVersion(),
        TransferFormat.JSON,
        chunk -> {
  ...
  private static String readZipEntryAsString(InputStream zipInputStream, ZipEntry entry)
      throws IOException {
    if (entry.isDirectory()) {
