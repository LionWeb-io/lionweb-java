...
public class JsonSchemaValidation {
  ...
  private static JsonSchema getGenericSerializationSchema() throws IOException {
    ...
    try (InputStream schemaStream =
        inputStreamFromClasspath("serialization/generic-serialization.schema.json")) {
  ...
  private static JsonSchema getLioncoreSerializationSchema() throws IOException {
    ...
    try (InputStream schemaStream =
        inputStreamFromClasspath("serialization/lioncore.serialization.schema.json")) {
      JsonSchema schema = schemaFactory.getSchema(schemaStream);
      return schema;
    }
  }
  ...
  private static JsonNode loadJson(String path) throws IOException {
    ...
    try (InputStream jsonStream = inputStreamFromClasspath(path); ) {
