...
public class ProtoBufSerialization extends AbstractSerialization {
  ...
  public BulkImport serializeBulkImport(List<BulkImportElement> elements) {
    ...
    Function<MetaPointer, Integer> metaPointerIndexer =
        metaPointer -> {
          if (metaPointerDefs.containsKey(metaPointer)) {
            return metaPointerDefs.get(metaPointer).getIndex();
          }
          MetaPointerDef metaPointerDef =
              MetaPointerDef.newBuilder()
    ...
    elements.forEach(
        bulkImportElement -> {
          ...
          bulkImportElementBuilder.setMetaPointerIndex(
              metaPointerIndexer.apply(bulkImportElement.containment));
                            ...
                            p ->
                                nodeBuilder.addContainments(
                                    Containment.newBuilder()
                                        ...
                                        .setMetaPointerIndex(
                                            metaPointerIndexer.apply(p.getMetaPointer()))
                                        .build()));
                    n.getReferences()
                        .forEach(
                            p ->
        ...
        .forEach(
            ul -> {
              chunkBuilder.addLanguages(
                  Language.newBuilder().setKey(ul.getKey()).setVersion(ul.getVersion()).build());
            });
    final Map<MetaPointer, MetaPointerDef> metaPointerDefs = new HashMap<>();
