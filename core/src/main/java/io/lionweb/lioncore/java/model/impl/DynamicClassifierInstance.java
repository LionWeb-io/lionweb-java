...
public abstract class DynamicClassifierInstance<T extends Classifier<T>> implements ClassifierInstance {
    protected String id;
    ...
    public List<AnnotationInstance> getAnnotations(Annotation annotation) {
        return annotations.stream().filter(a -> a.getAnnotationDefinition() == annotation).collect(Collectors.toList());
    }
