...
public class SerializedClassifierInstance {
  ...
  public String getPropertyValue(String propertyKey) {
    if (schema != null) {
      // Linear scan by key string; MetaPointer.getKey() is a plain String field, so .equals() is correct.
  ...
  /**
   * WARNING: this will always append/overwrite the property, even if one entry with the same
   * MetaPointer already exists.
   *
   * <p>It is however slightly faster than the (safer) {@link #setPropertyValue}.
   *
  ...
  /**
   * Updates or adds a {@link SerializedPropertyValue}. If a property with the same MetaPointer
   * already exists it is replaced; otherwise the property is appended.
   */
  ...
  /**
   * WARNING: always appends/overwrites the containment, even if one entry with the same
   * MetaPointer already exists.
   *
   * <p>Slightly faster than safer alternatives.
   */
  ...
  /**
   * Remove all containments. This is useful when creating a partition, as they cannot be created
   * with children.
   */
  ...
  /**
   * WARNING: always appends/overwrites the reference, even if one entry with the same MetaPointer
   * already exists.
   */
  ...
  public void addReferenceValue(
    ...
    if (entry.isPresent()) {
      List<SerializedReferenceValue.Entry> newValue =
          new ArrayList<>(entry.get().getValue());
  ...
  public void addReferenceValue(
    ...
    if (index < 0) throw new IllegalArgumentException("Index must be greater than or equal to zero");
    Objects.requireNonNull(metaPointer);
    Objects.requireNonNull(referenceValue);
    if (schema != null) {
  ...
  public void setAnnotations(@Nonnull List<String> annotationIDs) {
    ...
    if (annotations == null) {
      annotations = new ArrayList<>(annotationIDs.size());
    } else {
      annotations.clear();
    }
    annotations.addAll(annotationIDs);
  }
