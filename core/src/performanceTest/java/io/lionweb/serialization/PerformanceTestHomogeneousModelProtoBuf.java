 ...
 */
@Tag("performance")
public class PerformanceTestHomogeneousModelProtoBuf {
  ...
  @Test
  public void deserializeHomogeneousModelProtoBufMemoryAllocation() throws Exception {
    byte[] pbBytes = buildProtoBufBytes();
    ProtoBufSerialization pbs = new ProtoBufSerialization();
    System.out.println(
        "Homogeneous model ("
  ...
  private byte[] buildProtoBufBytes() {
    ...
    for (int i = 0; i < NODE_COUNT; i++) {
      SerializedClassifierInstance sci = new SerializedClassifierInstance("n-" + i, MP_CLASSIFIER);

      // 5 properties
      sci.unsafeAppendPropertyValue(SerializedPropertyValue.get(MP_NAME, "method_" + i));
      sci.unsafeAppendPropertyValue(
          SerializedPropertyValue.get(MP_VISIBILITY, VISIBILITIES[i & 3]));
      sci.unsafeAppendPropertyValue(
      ...
      int overridesIdx = (i + NODE_COUNT / 2) % NODE_COUNT;
