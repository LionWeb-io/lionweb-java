...
class RepositoryData {
    ...
    void store(List<SerializedClassifierInstance> newNodes) {
        ...
        Stream<SerializedClassifierInstance> heads = newNodes.stream()
                .filter(n -> n.getParentNodeID() == null || !newNodesByID.containsKey(n.getParentNodeID()));
    ...
    private void store(SerializedClassifierInstance serializedClassifierInstance, Map<String, SerializedClassifierInstance> newNodesByID) {
        if (nodesByID.get(serializedClassifierInstance.getID())!= serializedClassifierInstance) {
            ...
            serializedClassifierInstance.getChildren().forEach(c ->
                    store(newNodesByID.get(c), newNodesByID));
        }
    }
