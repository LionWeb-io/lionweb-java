...
public class LocalInMemoryDeltaChannelTest {
    ...
    public void setProperty() {
        ...
        DeltaSynchronizer synchronizer = new DeltaInMemorySynchronizer(channel, server, "repo1", LionWebVersion.v2024_1);
        ...
        assertEquals("MyLanguage",
                server.retrieve("repo1", Collections.singletonList("l1"), 0).get(0).
                        getPropertyValue(MetaPointer.from(LionCoreBuiltins.getINamed()
                                .getPropertyByName("name"))));

        l1.setName("MyOtherLanguage");
        assertEquals("MyOtherLanguage",
                server.retrieve("repo1", Collections.singletonList("l1"), 0).get(0).
                        getPropertyValue(MetaPointer.from(LionCoreBuiltins.getINamed()
                                .getPropertyByName("name"))));
    }
